...
create policy "Room creator can manage draft slots"
  on public.draft_slots for all using (
    exists (select 1 from public.rooms
      where rooms.id = draft_slots.room_id and rooms.created_by = auth.uid())
  );
create policy "Captains can update their team slots"
  on public.draft_slots for update using (
    exists (select 1 from public.room_participants rp
      where rp.room_id = draft_slots.room_id and rp.user_id = auth.uid()
      and ((rp.role = 'captain_a' and draft_slots.team_id = 'team_a')
